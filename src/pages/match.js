...
/* =========================================================
   CLUB HELPERS - Use shortName if available
========================================================= */
...
const FORMATIONS = {
  '4-4-2': {
    name: '4-4-2',
    positions: [
      { x: 8, y: 50 }, { x: 23, y: 18 }, { x: 23, y: 39 },
      { x: 23, y: 61 }, { x: 23, y: 82 }, { x: 42, y: 20 },
      { x: 42, y: 42 }, { x: 42, y: 58 }, { x: 42, y: 80 },
      { x: 62, y: 36 }, { x: 62, y: 64 },
    ],
  },
  '4-3-3': {
    ...
    positions: [
      { x: 8, y: 50 }, { x: 23, y: 18 }, { x: 23, y: 39 },
      { x: 23, y: 61 }, { x: 23, y: 82 }, { x: 42, y: 30 },
      { x: 42, y: 50 }, { x: 42, y: 70 }, { x: 64, y: 20 },
      { x: 64, y: 50 }, { x: 64, y: 80 },
    ],
  },
  '3-5-2': {
    name: '3-5-2',
    positions: [
      { x: 8, y: 50 }, { x: 23, y: 30 }, { x: 23, y: 50 },
      { x: 23, y: 70 }, { x: 42, y: 20 }, { x: 42, y: 39 },
      { x: 42, y: 50 }, { x: 42, y: 61 }, { x: 42, y: 80 },
      { x: 64, y: 36 }, { x: 64, y: 64 },
    ],
  },
  '5-3-2': {
    name: '5-3-2',
    positions: [
      { x: 8, y: 50 }, { x: 20, y: 15 }, { x: 20, y: 32 },
      { x: 20, y: 50 }, { x: 20, y: 68 }, { x: 20, y: 85 },
  ...
  '4-2-3-1': {
    ...
    positions: [
      { x: 8, y: 50 }, { x: 23, y: 18 }, { x: 23, y: 39 },
      { x: 23, y: 61 }, { x: 23, y: 82 }, { x: 38, y: 35 },
      { x: 38, y: 65 }, { x: 52, y: 20 }, { x: 52, y: 50 },
      { x: 52, y: 80 }, { x: 68, y: 50 },
    ],
  },
};
...
const TACTICS = {
  'Tiki-Taka': { name: 'Tiki-Taka', passChance: 0.6, dribbleChance: 0.2, shootChance: 0.2, pressIntensity: 0.7 },
  'Counter Attack': { name: 'Counter Attack', passChance: 0.3, dribbleChance: 0.4, shootChance: 0.3, pressIntensity: 0.4 },
  'High Press': { name: 'High Press', passChance: 0.4, dribbleChance: 0.3, shootChance: 0.3, pressIntensity: 0.9 },
  'Park the Bus': { name: 'Park the Bus', passChance: 0.5, dribbleChance: 0.2, shootChance: 0.3, pressIntensity: 0.2 },
  'Wing Play': { name: 'Wing Play', passChance: 0.4, dribbleChance: 0.35, shootChance: 0.25, pressIntensity: 0.5 },
};
...
export default function MatchPage() {
  ...
  useEffect(() => {
    ...
    async function loadMatch() {
      try {
        ...
        setHomeBench(preparedHome.filter((p) => !startingHome.some((s) => String(playerId(s)) === String(playerId(p)))));
        setAwayBench(preparedAway.filter((p) => !startingAway.some((s) => String(playerId(s)) === String(playerId(p)))));
  ...
  useEffect(() => {
    ...
    playerMeshesRef.current.home = formationPositions.map((pos) => {
      const x = ((pos.x - 50) / 50) * 15;
      const z = ((pos.y - 50) / 50) * 10;
      return createPlayer(x, z, 0x3b82f6);
    });

    playerMeshesRef.current.away = formationPositions.map((pos) => {
      const x = ((50 - pos.x) / 50) * 15;
      const z = ((50 - pos.y) / 50) * 10;
      return createPlayer(x, z, 0xef4444);
    });

    playerTargetsRef.current.home = playerMeshesRef.current.home.map((p) => ({ x: p.position.x, z: p.position.z }));
    playerTargetsRef.current.away = playerMeshesRef.current.away.map((p) => ({ x: p.position.x, z: p.position.z }));
  ...
  /* =======================================================
     SIMULATE PLAYER MOVEMENT + LIVE STATS
  ======================================================= */
  ...
  const simulatePlayerMovement = useCallback(() => {
    ...
    if (random < tacticData.passChance) {
      ...
      if (teammateIndex !== playerIndex && playerMeshes[teammateIndex]) {
        ...
        if (team === 'home') homeStatsData.passes += 1;
        else awayStatsData.passes += 1;
        return;
      }
    }
    ...
    if (random < tacticData.shootChance && distanceToGoal < 10) {
      ...
      if (Math.random() > saveChance) {
        ...
        const goalEvent = {
          ...
          team,
          minute: matchMinute,
          playerId: playerId(players[playerIndex]),
        ...
        if (opposingTeam === 'home') homeStatsData.saves += 1;
        else awayStatsData.saves += 1;
    ...
    if (currentPlayerData) {
      ...
      const stamina = team === 'home' ? staminaRef.current.home : staminaRef.current.away;
      if (stamina[pid]) {
        stamina[pid] = clamp(stamina[pid] - 0.5, 0, 100);
        setPlayerStamina({ home: staminaRef.current.home, away: staminaRef.current.away });
      }
    }
  ...
  const saveMatchState = useCallback(async ({
    ...
    await setDoc(matchRef, {
      ...
      result: { homeScore: safeNumber(homeScoreValue, 0), awayScore: safeNumber(awayScoreValue, 0) },
  ...
  const simulateMinute = useCallback(async (minute) => {
    ...
    try {
      ...
      await saveMatchState({
        ...
        eventsValue: events,
        homeStatsValue: statsRef.current.home,
        awayStatsValue: statsRef.current.away,
        statusValue: 'live',
  ...
  const startMatch = useCallback(async () => {
    ...
    try {
      setSavingMatch(true);
      await saveMatchState({
        minute: 0, homeScoreValue: homeScore, awayScoreValue: awayScore,
  ...
  const changeFormation = async (value) => {
    ...
    setHomeXI(startingHome);
    setAwayXI(startingAway);
    setHomeBench(homeSquad.filter((p) => !startingHome.some((s) => String(playerId(s)) === String(playerId(p)))));
    setAwayBench(awaySquad.filter((p) => !startingAway.some((s) => String(playerId(s)) === String(playerId(p)))));
  ...
  const makeSubstitution = async () => {
    ...
    const playerIn = bench.find((p) => String(playerId(p)) === String(selectedSubIn));
    const playerOut = currentXI.find((p) => String(playerId(p)) === String(selectedSubOut));
    ...
    const nextXI = currentXI.map((p) =>
      String(playerId(p)) === String(playerId(playerOut)) ? playerIn : p
    );
    const nextBench = [
      ...bench.filter((p) => String(playerId(p)) !== String(playerId(playerIn))),
      playerOut,
    ];
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content={`Live match between ${getClubName(homeClub)} and ${getClubName(awayClub)}`} />
      </Head>

      <main className={styles.page}>
        {/* HEADER */}
        <header className={styles.header}>
          <button type="button" className={styles.backButton} onClick={() => router.push('/fixtures')}>
            ← Fixtures
          </button>
          <div>
            <span className={styles.competition}>{fixture.leagueName || fixture.competition || 'MATCH'}</span>
            <h1>Match Centre</h1>
          </div>
        ...
        {!userIsParticipant && (
          <section className={styles.warning}>
            ...
            <p>Your current club is not participating in this match. You can view the latest score and events, but you cannot play it.</p>
          </section>
        )}
        ...
        {userIsParticipant && (
          <section className={styles.controls}>
            {matchStatus === 'ready' && (
              <button type="button" className={styles.primaryButton} onClick={startMatch} disabled={savingMatch}>
                ▶ START MATCH
              </button>
            )}
            {matchStatus === 'live' && (
            ...
            {matchStatus === 'half-time' && (
              <button type="button" className={styles.primaryButton} onClick={continueSecondHalf} disabled={savingMatch}>
                ▶ START SECOND HALF
              </button>
            )}
            <button type="button" onClick={() => setShowFormation((prev) => !prev)}>📋 Formation</button>
            <button type="button" onClick={() => setShowTactics((prev) => !prev)}>⚙ Tactics</button>
            <button type="button" onClick={() => setShowSubs((prev) => !prev)}>🔄 Substitutions</button>
          </section>
        )}
        ...
        {halfTimeShown && (
          <section className={styles.halfTime}>
            ...
            <button type="button" onClick={continueSecondHalf} disabled={savingMatch}>Continue</button>
          </section>
        )}
        ...
        {showFormation && userIsParticipant && (
          <section className={styles.panel}>
            <div className={styles.panelHeader}><h2>Formation</h2><span>{formation}</span></div>
            <div className={styles.mentalityGrid}>
              {Object.keys(FORMATIONS).map((value) => (
                <button key={value} type="button" className={formation === value ? styles.active : ''} onClick={() => changeFormation(value)}>
                  {value}
                </button>
              ))}
            </div>
        ...
        {showTactics && userIsParticipant && (
          <section className={styles.panel}>
            <div className={styles.panelHeader}><h2>Tactics</h2><span>{tactic}</span></div>
            <div className={styles.mentalityGrid}>
              {Object.keys(TACTICS).map((value) => (
                <button key={value} type="button" className={tactic === value ? styles.active : ''} onClick={() => changeTactic(value)}>
                  {value}
                </button>
              ))}
            </div>
            <div className={styles.panelHeader}><h2>Mentality</h2></div>
            <div className={styles.mentalityGrid}>
              {['defensive', 'balanced', 'attacking'].map((value) => (
                <button key={value} type="button" className={mentality === value ? styles.active : ''} onClick={() => changeMentality(value)}>
        ...
        {showSubs && userIsParticipant && (
          <section className={styles.panel}>
            <div className={styles.panelHeader}><h2>Substitutions</h2><span>{substitutionsUsed}/{MAX_SUBSTITUTIONS}</span></div>
            <div className={styles.subRow}>
              <label>Player IN (from bench)</label>
              <select value={selectedSubIn} onChange={(event) => setSelectedSubIn(event.target.value)}>
                <option value="">Select player IN</option>
                {(userTeam === 'home' ? homeBench : awayBench).map((player) => (
                  <option key={String(playerId(player))} value={String(playerId(player))}>
            ...
            <div className={styles.subRow}>
              ...
              <select value={selectedSubOut} onChange={(event) => setSelectedSubOut(event.target.value)}>
                <option value="">Select player OUT</option>
                {(userTeam === 'home' ? homeXI : awayXI).map((player) => (
                  <option key={String(playerId(player))} value={String(playerId(player))}>
                    {getPlayerName(player)} · OVR {getPlayerOverall(player)} · STA {Math.round(playerStamina[userTeam]?.[playerId(player)] || 100)}%
                  </option>
                ))}
              </select>
            </div>
            <button type="button" onClick={makeSubstitution} disabled={substitutionsUsed >= MAX_SUBSTITUTIONS || !selectedSubIn || !selectedSubOut}>
              Make Substitution
            </button>
          </section>
        )}
        ...
        <section className={styles.eventsSection}>
          <div className={styles.sectionHeader}>
            <div><span>LIVE FEED</span><h2>Match Events</h2></div>
        ...
        <section className={styles.lineups}>
          <div className={styles.lineupCard}>
            <div className={styles.sectionHeader}><h2>{getClubName(homeClub)}</h2><span>{homeXI.length}/11 · {formation}</span></div>
            <div className={styles.playerList}>
              {homeXI.map((player) => (
                <div key={String(playerId(player))} className={styles.player}>
                  {getPlayerPhoto(player) ? <img src={getPlayerPhoto(player)} alt="" /> : <span>⚽</span>}
                  <div><strong>{getPlayerName(player)}</strong><small>{normalizePosition(getPlayerPosition(player))} · OVR {getPlayerOverall(player)}</small></div>
                  <div className={styles.playerMeta}>
                    <span className={styles.playerRating}>{getPlayerOverall(player)}</span>
                    <span className={styles.staminaBar}>
                      <span className={styles.staminaFill} style={{ width: `${Math.round(playerStamina.home?.[playerId(player)] || 100)}%` }} />
                    </span>
                    <small>{Math.round(playerStamina.home?.[playerId(player)] || 100)}%</small>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className={styles.lineupCard}>
            <div className={styles.sectionHeader}><h2>{getClubName(awayClub)}</h2><span>{awayXI.length}/11 · {formation}</span></div>
            <div className={styles.playerList}>
              {awayXI.map((player) => (
                <div key={String(playerId(player))} className={styles.player}>
                  {getPlayerPhoto(player) ? <img src={getPlayerPhoto(player)} alt="" /> : <span>⚽</span>}
                  ...
                  <div className={styles.playerMeta}>
                    <span className={styles.playerRating}>{getPlayerOverall(player)}</span>
                    <span className={styles.staminaBar}>
                      <span className={styles.staminaFill} style={{ width: `${Math.round(playerStamina.away?.[playerId(player)] || 100)}%` }} />
                    </span>
                    <small>{Math.round(playerStamina.away?.[playerId(player)] || 100)}%</small>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className={styles.matchInfo}>
          <div><span>STADIUM</span><strong>{fixture.stadium || getClubStadium(homeClub)}</strong></div>
          <div><span>FORMATION</span><strong>{formation}</strong></div>
          <div><span>TACTIC</span><strong>{tactic}</strong></div>
          <div><span>MATCH ID</span><strong>{matchId}</strong></div>
        </section>
      </main>
    </>
  );
