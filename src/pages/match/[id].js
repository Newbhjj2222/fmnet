...
function normalizeId(value) {
  ...
  if (typeof value === "object") {
    return String(
      value.id || value.clubId || value.teamId || value.value || ""
    );
  }
  return String(value);
}
...
function getUserIdFromUser(user) {
  return safeString(
    user?.uid || user?.id || user?.userId || user?._id
  );
}
...
function calculatePlayerRating(player) {
  ...
  const directRating = firstValue(player, [
    "rating",
    "overall",
    "overallRating",
    "overall_rating",
  ...
  const nestedRating = firstValue(ratings, [
    "rating",
    "overall",
    "overallRating",
    "overall_rating",
  ...
  return Math.max(1, Math.min(99, Math.round(average)));
}
...
function positionCompatibility(playerPosition, wantedPosition) {
  ...
  if (allowed.includes(position)) return 80;

  if (wanted === "CB" && ["DM", "RB", "LB"].includes(position)) {
    return 55;
  }
...
function assignFormationPositions(players, formation, side = "home") {
  ...
  slots.forEach(slot => {
    ...
    remaining.forEach((player, index) => {
      ...
      const score = compatibility * 1000 + rating;
    ...
    assigned.push({
      ...
      role: normalizePosition(player.role || player.position),
      x,
      y: slot.y,
...
function normalizePlayer(player = {}, index = 0) {
  ...
  const position = normalizePosition(
    player.position || player.preferredPosition || player.role || "CM"
  );

  return {
    id: String(
      player.id || player.playerId || player.uid || `player-${index + 1}`
    ),
    name:
      player.name ||
      player.displayName ||
    ...
    position,
    role: normalizePosition(player.role || player.position || "CM"),
    rating: calculatePlayerRating(player),
    ...
    shooting: safeNumber(
      player.shooting ?? ratings.shooting ?? ratings.shoot,
      65
    ),
    dribbling: safeNumber(
      player.dribbling ?? ratings.dribbling ?? ratings.dribble,
      68
    ),
    ...
    strength: safeNumber(
      player.strength ?? ratings.strength ?? ratings.physical,
      70
    ),
    vision: safeNumber(player.vision ?? ratings.vision, 70),
    goalkeeping: safeNumber(
      player.goalkeeping ?? ratings.goalkeeping ?? ratings.gk,
      60
    ),
    ...
    acceleration: safeNumber(
      player.acceleration ?? ratings.acceleration,
      68
    ),
...
function normalizeClub(club, fallbackId) {
  return {
    ...
    name: club?.name || club?.clubName || club?.title || "Unknown Club",
    logo:
      club?.logo ||
      club?.logoUrl ||
      club?.image ||
...
function findManagedClubInDatabase(clubs, username, userId) {
  ...
  if (!wantedUsername && !wantedUserId) return null;
  ...
  if (Array.isArray(explicitBench) && explicitBench.length) {
    ...
    source = allPlayers.filter(
      player =>
        ...
        !startingIds.has(
          String(player.id || player.playerId || player.uid)
        )
    );
  }
...
function formatClock(minute, second) {
  ...
  return `${String(safeMinute).padStart(2, "0")}:${String(
    safeSecond
  ).padStart(2, "0")}`;
}
...
function normalizeEventType(event) {
  return safeString(
    event?.type || event?.eventType || event?.action || event?.name
  )
    .toLowerCase()
    .replace(/[\s-]+/g, "_");
}
...
function calculateLiveStats(snapshot, side) {
  ...
  sideEvents.forEach(event => {
    ...
    if (calculateShots && type.includes("shot")) shots += 1;
    ...
    if (calculateTackles && type.includes("tackle")) tackles += 1;
    if (calculateInterceptions && type.includes("interception"))
      interceptions += 1;
    if (calculateCorners && type.includes("corner")) corners += 1;
    if (calculateSaves && type.includes("save")) saves += 1;
    if (calculateFouls && type.includes("foul")) fouls += 1;
  });
...
export default function MatchPage() {
  ...
  const attachEngine = useCallback(
    engine => {
      ...
      engine.subscribe(nextSnapshot => {
        ...
        if (nextSnapshot.status === "created" && !nextSnapshot.ball) {
          nextSnapshot = {
            ...nextSnapshot,
            ball: { x: 50, y: 50 },
          };
        ...
        if (
          ...
          nextSnapshot.away.bench.length > 0
        ) {
          setAwayBenchPlayers(
        ...
        if (controlledTeam?.formation) {
          setFormation(controlledTeam.formation);
        }
  ...
  useEffect(() => {
    ...
    async function loadMatch() {
      try {
        ...
        /* ---------- Formation ---------- */
        const homeFormation =
          match?.home?.formation || homeClub.formation || "4-3-3";
        const awayFormation =
          match?.away?.formation || awayClub.formation || "4-3-3";
        ...
        homeStarting = assignFormationPositions(
          homeStarting,
          homeFormation,
          "home"
        );
        awayStarting = assignFormationPositions(
          awayStarting,
          awayFormation,
          "away"
        );
        ...
        const managedStarting = isHome
          ? homeStarting
          : isAway
          ? awayStarting
          : [];

        const managedBenchPlayers = isHome
          ? homeBench
          : isAway
          ? awayBench
          : [];
        ...
        setManagedTeamName(
          isHome ? homeClub.name : isAway ? awayClub.name : "No Team"
        );

        /* ---------- Tactics ---------- */
        const managedClub = isHome
          ? homeClub
          : isAway
          ? awayClub
          : null;

        const managedMatchSide = isHome
          ? match?.home
          : isAway
          ? match?.away
          : null;
        ...
        matchConfigRef.current = {
          ...
          metadata: {
            ...
            managedClubId:
              ...
              (isHome ? homeClubId : isAway ? awayClubId : null),
  ...
  const startMatch = useCallback(async () => {
    ...
    try {
      ...
      await engine.setUserTactics(tactics);
      await engine.setFormation(formation);

      if (typeof engine.setBallPosition === "function") {
        await engine.setBallPosition(50, 50);
      }
  ...
  }, [managedStartingXI.length, tactics, formation]);
  ...
  async function saveTactics() {
    ...
    try {
      ...
      if (
        ...
        const currentAway =
          state?.away?.players ||
          matchConfigRef.current?.away?.players ||
          [];
  ...
  const saveStartingXI = useCallback(async () => {
    ...
    try {
      ...
      const positionedXI = assignFormationPositions(
        managedStartingXI,
        formation,
        side
      );

      const homeXI = side === "home" ? positionedXI : config.home.players;
      const awayXI = side === "away" ? positionedXI : config.away.players;
      ...
      setManagedStartingXI(positionedXI);
      ...
      matchConfigRef.current = {
        ...
        home: {
          ...
          bench: homeBench,
          formation:
            side === "home" ? formation : config.home.formation,
        },
        away: {
          ...config.away,
          players: awayXI,
          bench: awayBench,
          formation:
            side === "away" ? formation : config.away.formation,
        },
        ball: { x: 50, y: 50 },
      };
  ...
  const allPlayersOnPitch = useMemo(() => {
  ...
  const substitutionsUsed = safeNumber(
    managedSide === "away"
      ? away?.substitutionsUsed
      : home?.substitutionsUsed,
    safeNumber(snapshot?.substitutions?.[managedSide || "home"], 0)
  );
          ...
          {allPlayersOnPitch.map(player => {
            ...
            return (
              ...
              >
                ...
                <small className={styles.playerPosition}>
                  {normalizePosition(
                    player.position || player.formationPosition
                  )}
                </small>
                <small className={styles.playerRating}>
                  {safeNumber(
                    player.rating,
            ...
            {activeTab === "subs" && managedSide && (
              <div className={styles.substitutionPanel}>
                ...
                <div className={styles.formGroup}>
                  ...
                  {liveManagedBench.length === 0 && (
                    <p className={styles.empty}>
                      Nta bakinnyi kuri bench. Reba niba club
                      ifite abakinnyi barenze 11 muri database.
                    </p>
                  )}
                </div>
                ...
                <div className={styles.substitutionInfo}>
                  <p>
                    Substitutions used:{" "}
                    <strong>{substitutionsUsed}</strong> / 5
                  </p>
                  <p>
