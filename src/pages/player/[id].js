...
function calculateCounterOffer(
  ...
  /*
   * AI is harder to negotiate with
   * when player overall is high.
   */
  ...
  /*
   * Never make counter lower
   * than the user's offer.
   */
...
export default function PlayerPage() {
  ...
  const [negotiating, setNegotiating] =
    useState(false);
  ...
  function openNegotiation() {
    ...
    if (isOwnPlayer) {
      toast.error(
        'This player is already in your club.'
      );
      return;
    }
    ...
    if (budget <= 0) {
      toast.error(
        'Your transfer budget is empty.'
      );
      return;
    }

    /*
     * If AI already sent counter offer,
     * show that amount first.
     */

    if (aiCounterOffer) {
      setOfferAmount(
        String(aiCounterOffer)
      );
    } else {
      setOfferAmount(
        String(
          Math.round(
            askingPrice(player) * 0.85
          )
        )
      );
    }
  ...
  async function submitNegotiation() {
    ...
    if (
      ...
      toast.error(
        `Your transfer budget is only €${money(budget)}.`
      );
      return;
    }
    ...
    if (
      ...
      toast.error(
        `The offer is too low. Try at least €${money(
          Math.round(
            asking *
              MIN_OFFER_RATIO
          )
    ...
    if (
      ...
      toast.error(
        `The offer is unusually high compared with the asking price.`
      );
      return;
    }
    ...
    try {
      ...
      const offerId =
        `${careerData.currentClub}-${player.id}-${Date.now()}`;

      /*
       * IMPORTANT:
       *
       * Game time comes from careerData.currentDate.
       * Real Date.now() is used ONLY for the unique ID.
       */
      ...
      const offer = {
        ...
        negotiationRound:
          latestMyOffer
            ? safeNumber(
                latestMyOffer.negotiationRound,
                1
              ) + 1
            : 1,
      };
      ...
      /*
       * Save the offer inside
       * the player's transferOffers array.
       */
      ...
      /*
       * Also update local career budget
       * only as a reservation.
       *
       * The money should NOT permanently
       * leave the club until the transfer
       * is accepted.
       */
      ...
      toast.error(
        'Could not submit transfer offer.'
      );
    } finally {
  ...
  async function acceptCounterOffer() {
    ...
    try {
      ...
      if (index === -1) {
        toast.error(
          'This negotiation is no longer available.'
        );
        return;
      }
      ...
      toast.success(
        `${playerName(
          player
        )} transfer agreed for €${money(
          aiCounterOffer
        )}.`
      );
    } catch (error) {
      console.error(
        'Counter acceptance error:',
        error
      );
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content={`Player profile for ${playerName(
            player
          )}`}
        />
      </Head>
              ...
              <div>
                ...
                {aiCounterOffer && (
                  ...
                  >
                    ...
                    <p>
                      The AI club has responded
                      with a new transfer fee.
                    </p>

                    <div
                      className={
                        styles.counterActions
                      }
                    >
                      <button
                        type="button"
                        className={
                          styles.acceptCounter
                        }
                        disabled={
                          negotiating
                        }
        ...
        {latestMyOffer && (
          ...
          >
            ...
            <div>
              ...
              <strong>
                {formatDate(
                  latestMyOffer.gameDate ||
                    latestMyOffer.createdAt
                )}
              </strong>
            </div>
        ...
        {showNegotiation && (
          ...
          >
            ...
            >
              ...
              <div
                className={
                  styles.gameDateNotice
                }
              >
                📅 This negotiation uses
                the game calendar:
                <strong>
                  {formatDate(
                    gameDate
