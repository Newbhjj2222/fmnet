...
/*
 * Firestore batched writes are limited to 500 operations.
 * We use a smaller safe limit.
 */
...
/* =========================================================
   SEEDED RANDOM
   Same league + same season = same schedule.
========================================================= */
...
function seededRandom(seed) {
  ...
  for (let i = 0; i < safeSeed.length; i++) {
    value =
      (value * 31 + safeSeed.charCodeAt(i)) %
      2147483647;
  }
...
function generateSeasonDates(
  ...
  const start = randomSeasonStart(
    seasonYear,
    league.id
  );

  let end = randomSeasonEnd(
    seasonYear,
    league.id
  );
...
function getMatchHour(
  leagueId,
  round,
  index
) {
  const random = seededRandom(
    `${leagueId}-${round}-${index}`
  );
...
/* =========================================================
   ...
   NOTE:
   This function ONLY GENERATES data.
...
function generateFriendlyFixture({
  userId,
  currentClub,
  opponent,
  seasonYear,
...
export default function FixturesPage({
  ...
  /*
   * IMPORTANT:
   * This is the ONLY fixture state used by UI.
   * It contains data coming from Firestore.
   */
  ...
  useEffect(() => {
    ...
    const unsubscribe =
      onSnapshot(
        ...
        (error) => {
          console.error(
            'Fixtures realtime listener error:',
            error
          );

          toast.error(
            'Live fixtures disconnected'
          );
        }
      );
  ...
  /* =======================================================
     GENERATE + SAVE FIXTURES
     
     IMPORTANT:
     - Generates only if season has no league matches.
     - Saves directly to Firestore.
     - UI never uses generated array directly.
======================================================== */

  const ensureSeasonFixtures =
    useCallback(
      async () => {
        if (
          !user ||
        ...
        try {
          ...
          if (
            ...
            console.log(
              `Season ${seasonYear} already has ${existingSnapshot.size} fixtures.`
            );
          ...
          toast.success(
            `${generated.length} fixtures generated and saved`
          );
        } catch (error) {
          console.error(
            'Season fixture generation error:',
            error
          );

          toast.error(
            'Could not generate season fixtures'
          );
        } finally {
          setIsGenerating(
            false
  ...
  useEffect(() => {
    ...
    /*
     * Only generate if there are no
     * database fixtures for this season.
     */
