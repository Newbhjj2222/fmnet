...
export async function getServerSideProps() {
  try {
    ...
    const countries = countriesSnapshot.docs.map((countryDoc) => ({
      id: countryDoc.id,
      ...countryDoc.data(),
    }));
    ...
    console.error('Players SSR error:', error);
    return {
      props: {
        initialPlayers: [],
        initialClubs: [],
...
export default function PlayersPage({
  ...
  const [negotiationType, setNegotiationType] = useState('transfer');
  ...
  /* =======================================================
     REAL-TIME CLUB BUDGET LISTENER
     Only source of truth: clubs/{clubId}
  ======================================================= */
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        console.error('Club budget realtime error:', error);
      }
    );
  ...
  const currentClubPlayers = useMemo(() => {
    ...
    return players.filter(
      (player) => getClubId(player) === currentClubId
    );
  ...
  const scoutSuggestions = useMemo(() => {
    const budget = transferBudget; // from club doc

    return players
      .filter((player) => getClubId(player) !== currentClubId)
      .map((player) => {
        ...
        if (
          normalize(getTransferStatus(player)).includes('listed')
        ) {
          score += 15;
        }
      ...
      .slice(0, MAX_SCOUT_RESULTS);
  }, [players, currentClubId, transferBudget]);
  ...
  const filteredPlayers = useMemo(() => {
    let source =
      activeTab === 'squad' ? currentClubPlayers : transferMarket;
    ...
    if (searchValue) {
      source = source.filter((player) => {
        return (
          ...
          normalize(getPosition(player)).includes(searchValue) ||
          normalize(getClubName(player)).includes(searchValue) ||
          normalize(
    ...
    if (positionFilter !== 'all') {
      source = source.filter(
        (player) => getPosition(player) === positionFilter
      );
    }
    ...
    if (statusFilter !== 'all') {
      source = source.filter(
        (player) =>
          normalize(getTransferStatus(player)) ===
    ...
    if (clubFilter !== 'all') {
      source = source.filter(
        (player) => getClubId(player) === clubFilter
      );
    }
  ...
  const removeFromTransfer = async (player) => {
    const success = await updatePlayer(player.id, {
  ...
  const listForLoan = async (player) => {
    const success = await updatePlayer(player.id, {
      transferStatus: 'loan-listed',
      loanListedBy: currentClubId,
      loanListedAt: new Date().toISOString(),
    });
    if (success) {
      toast.success('Player added to loan list');
    }
  };
  ...
  const removeFromLoan = async (player) => {
    const success = await updatePlayer(player.id, {
      transferStatus: 'available',
      loanListedBy: null,
      loanListedAt: null,
    ...
    if (success) {
      toast.success('Player removed from loan list');
    }
  };
  ...
  const openNegotiation = (player) => {
    ...
    setOfferAmount(
      String(getAskingPrice(player) || getValue(player) || '')
    );
    setNegotiationType('transfer');
  ...
  const makeTransferOffer = async () => {
    if (!selectedPlayer || !currentClubId) return;

    const offer = safeNumber(offerAmount, 0);
    if (offer <= 0) {
    ...
    try {
      ...
      await updateDoc(doc(db, 'players', selectedPlayer.id), {
        transferOffers: arrayUnion(offerData),
        latestOffer: offerData,
      ...
      toast.success('Transfer offer submitted');
    } catch (error) {
  ...
  const makeLoanOffer = async () => {
    if (!selectedPlayer || !currentClubId) return;

    const fee = safeNumber(loanFee, 0);
    const duration = safeNumber(loanDuration, 1);

    if (duration <= 0) {
      toast.error('Invalid loan duration');
      return;
    }
    ...
    try {
      ...
      await updateDoc(doc(db, 'players', selectedPlayer.id), {
        loanOffers: arrayUnion(offerData),
        latestLoanOffer: offerData,
        updatedAt: serverTimestamp(),
      });
      ...
      console.error(error);
      toast.error('Could not submit loan offer');
    } finally {
      setSaving(false);
    }
  };
  ...
  const buyPlayer = async (player) => {
    ...
    const confirmed = window.confirm(
      `Submit an offer of €${money(price)} for ${
        player.name || player.fullName || 'this player'
      }?`
    );
  ...
  if (!currentClubId) {
    return (
      <>
        <Head>
          <title>Players & Transfers | Virtual Football Manager</title>
        </Head>
        <main className={styles.emptyPage}>
          <div className={styles.emptyIcon}>⚽</div>
          <h1>No Club Assigned</h1>
          <p>
            You need to take charge of a club before managing players and
            transfers.
          </p>
  ...
  const contractWarnings = currentClubPlayers.filter((player) => {
    const days = daysUntil(getContractEnd(player));
  ...
  return (
    <>
      <Head>
        <title>Players & Transfers | {currentClub?.name || 'Club'}</title>
        <meta
          name="description"
      ...
      <main className={styles.page}>
        ...
        <header className={styles.header}>
          <div className={styles.headerIdentity}>
            <div className={styles.clubLogo}>
              {currentClub?.logo ? (
                <img src={currentClub.logo} alt={currentClub.name} />
              ) : (
                '⚽'
              )}
            </div>
        ...
        <section className={styles.stats}>
          <div className={styles.statCard}>
            ...
            <div>
              <small>SQUAD</small>
              <strong>{currentClubPlayers.length}</strong>
          ...
          <div className={styles.statCard}>
            ...
            <div>
              <small>MARKET</small>
              <strong>{transferMarket.length}</strong>
          ...
          <div className={styles.statCard}>
            ...
            <div>
              <small>LISTED</small>
              <strong>{listedPlayers.length}</strong>
              <p>Your listed players</p>
            </div>
          </div>
          <div className={styles.statCard}>
            <span>⏳</span>
            <div>
              ...
              <strong>{contractWarnings.length}</strong>
              <p>Expiring within 180 days</p>
            </div>
          </div>
        </section>
        ...
        <nav className={styles.tabs}>
          <button
            className={activeTab === 'squad' ? styles.activeTab : ''}
            onClick={() => setActiveTab('squad')}
          >
            👥 My Squad
          </button>
          <button
            className={activeTab === 'market' ? styles.activeTab : ''}
            onClick={() => setActiveTab('market')}
          >
            🔄 Transfer Market
          </button>
          <button
            className={activeTab === 'scouting' ? styles.activeTab : ''}
        ...
        {/* FILTERS */}
        {(activeTab === 'squad' || activeTab === 'market') && (
          <section className={styles.filters}>
            ...
            >
              <option value="overall">Best Rated</option>
        ...
        {(activeTab === 'squad' || activeTab === 'market') && (
          <section className={styles.tableCard}>
            <div className={styles.tableHeader}>
              <div>
                <span>
                  {activeTab === 'squad' ? 'FIRST TEAM' : 'GLOBAL MARKET'}
                </span>
                <h2>{activeTab === 'squad' ? 'Squad' : 'Transfer Market'}</h2>
              </div>
                ...
                <tbody>
                  {filteredPlayers.length > 0 ? (
                    filteredPlayers.map((player) => {
                      const playerName =
                        player.name || player.fullName || 'Unnamed Player';
                      const status = getTransferStatus(player);
                      const isOwnPlayer =
                        getClubId(player) === currentClubId;
                      ...
                      return (
                        <tr key={player.id}>
                          ...
                          <td>{getPlayerAge(player) || '-'}</td>
                          <td>
                            <strong className={styles.overall}>
                              {getOverall(player) || '-'}
                            </strong>
                          </td>
                          <td>
                          ...
                          <td>€{money(getValue(player))}</td>
                          <td>
                            <strong>
                              €{money(getAskingPrice(player))}
                            </strong>
                          </td>
                          <td>
                            <span
                              className={`${styles.status} ${
                                styles[statusClass(status)]
                              }`}
                            >
                              {statusLabel(status)}
                            </span>
                            ...
                            <div className={styles.actions}>
                              ...
                              {isOwnPlayer ? (
                                <>
                                  {normalize(status) ===
                                    'transfer-listed' ||
                                  normalize(status) === 'listed' ? (
                                    <button
                                      type="button"
                                      className={styles.dangerAction}
                                      onClick={() => removeFromTransfer(player)}
                                    >
                                      Remove Transfer
                                    </button>
                                  ) : (
                                    <button
                                      type="button"
                                      onClick={() => listForTransfer(player)}
                                    >
                                      Add Transfer
                                    </button>
                                  )}

                                  {normalize(status) === 'loan-listed' ||
                                  normalize(status) === 'loan' ? (
                                    <button
                                      type="button"
                                      className={styles.dangerAction}
                                      onClick={() => removeFromLoan(player)}
                                    >
                                      Remove Loan
                                    </button>
                                  ) : (
                                    <button
                                      type="button"
                                      className={styles.loanAction}
                                      onClick={() => listForLoan(player)}
                                    >
                                      Add Loan
                                    </button>
                                  )}
        ...
        {/* SCOUTING */}
        {activeTab === 'scouting' && (
          <section className={styles.scouting}>
            <div className={styles.sectionHeading}>
              <div>
                <span>RECRUITMENT</span>
                <h2>Scout Recommendations</h2>
                <p>
                  Players your scouting department believes could improve the
                  squad.
                </p>
              </div>
              <span className={styles.scoutIcon}>🔭</span>
            </div>

            <div className={styles.scoutGrid}>
              {scoutSuggestions.map((player) => {
                const playerName =
                  player.name || player.fullName || 'Unnamed Player';
                return (
                  <article key={player.id} className={styles.scoutCard}>
                    <div className={styles.scoutTop}>
                      ...
                      <div>
                        ...
                        <strong>{player.scoutScore}</strong>
                      </div>
                    </div>
                    ...
                    <p>
                      {getPosition(player)} •{' '}
                      {getPlayerAge(player) || '-'} years
                    </p>

                    <div className={styles.scoutStats}>
                      <div>
                        <span>OVR</span>
                        <strong>{getOverall(player) || '-'}</strong>
                      </div>
                      <div>
                        <span>VALUE</span>
                        <strong>€{money(getValue(player))}</strong>
                      </div>
                      <div>
                        <span>CLUB</span>
                    ...
                    <div className={styles.scoutReason}>
                      {getPlayerAge(player) <= 23
        ...
        {activeTab === 'contracts' && (
          <section className={styles.tableCard}>
            <div className={styles.tableHeader}>
              <div>
                <span>SQUAD CONTRACTS</span>
                <h2>Contract Monitor</h2>
              </div>
            </div>

            <div className={styles.contractList}>
              {currentClubPlayers.map((player) => {
                const end = getContractEnd(player);
                const remaining = daysUntil(end);
                const warning = remaining !== null && remaining <= 180;

                return (
                  <div key={player.id} className={styles.contractRow}>
                    <div>
                      <strong>{player.name || player.fullName}</strong>
                    ...
                    <div>
                      ...
                      <strong
                        className={warning ? styles.contractWarning : ''}
                      >
                        {end
        ...
        {selectedPlayer && !showNegotiation && !showLoan && (
          <div
          ...
          >
            ...
            >
              ...
              <div className={styles.modalPlayerHeader}>
                ...
                <div>
                  <span>{getPosition(selectedPlayer)}</span>
                  <h2>
                    {selectedPlayer.name || selectedPlayer.fullName}
                  </h2>
                  <p>{getClubName(selectedPlayer)}</p>
                </div>
              </div>
              ...
              <div className={styles.detailsGrid}>
                <div>
                  <span>AGE</span>
                  <strong>{getPlayerAge(selectedPlayer) || '-'}</strong>
                </div>
                <div>
                  <span>OVERALL</span>
                  <strong>{getOverall(selectedPlayer) || '-'}</strong>
                </div>
                <div>
                  <span>MARKET VALUE</span>
                  <strong>€{money(getValue(selectedPlayer))}</strong>
                </div>
                <div>
                  <span>ASKING PRICE</span>
                  <strong>
                    €{money(getAskingPrice(selectedPlayer))}
                  </strong>
                </div>
                <div>
              ...
              <div className={styles.modalActions}>
                <button
                  type="button"
                  onClick={() => openNegotiation(selectedPlayer)}
                >
                  Negotiate Transfer
                </button>
                <button
                  type="button"
                  className={styles.loanAction}
                  onClick={() => openLoanOffer(selectedPlayer)}
                >
                  Make Loan Offer
                </button>
              </div>
        ...
        {showNegotiation && selectedPlayer && (
          <div
            className={styles.modalOverlay}
          ...
          >
            ...
            >
              ...
              <h2>{selectedPlayer.name || selectedPlayer.fullName}</h2>
              <p>
                {getClubName(selectedPlayer)} •{' '}
              ...
              <div className={styles.negotiationHint}>
                💡 You can offer below the asking price. The selling club may
        ...
        {showLoan && selectedPlayer && (
          ...
          >
            ...
            >
              ...
              <span className={styles.eyebrow}>LOAN NEGOTIATION</span>
              <h2>{selectedPlayer.name || selectedPlayer.fullName}</h2>
              <p>Request a temporary loan agreement.</p>
              ...
              <div className={styles.negotiationHint}>
                📋 The owning club will review your loan proposal before
                accepting it.
              </div>
              ...
              <button
                ...
                disabled={saving}
                className={styles.primaryButton}
                onClick={makeLoanOffer}
              >
                {saving ? 'Submitting...' : 'Submit Loan Offer'}
              </button>
            </div>
          </div>
        )}
