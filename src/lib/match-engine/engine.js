...
import {
  resolveShot,
  attemptShot,      // ✅ yongeweho
} from "./shooting";

import {
  attemptPass,     // ✅ yongeweho
} from "./passing";
...
export default class MatchEngine {
  constructor({
    ...
    this.score = {
      home:
        Number(initialScore?.home) || 0,
      away:
        Number(initialScore?.away) || 0,
    };
  ...
  resetPositions() {
    for (const team of [
      ...
      for (const player of team.players) {
        player.x =
          player.homeX * FIELD.width;

        player.y =
          player.homeY * FIELD.height;
        ...
        // ✅ gutangiza lastActionAt
        if (
          typeof player.lastActionAt !==
          "number"
        ) {
    ...
    this.ball.state = BALL_STATE.FREE;
  }
  ...
  kickoff() {
    const midfielders =
      this.home.players
        ...
        .sort(
          (a, b) =>
            Math.abs(a.x - FIELD.centerX) -
            Math.abs(b.x - FIELD.centerX)
        );
    ...
    if (player) {
      player.x = FIELD.centerX - 20;
      player.y = FIELD.centerY;

      setBallOwner(this.ball, player);
  ...
  update(dt) {
    ...
    // ✅ kongera lastActionAt kuri buri player
    this.tickPlayerActionTimers(safeDt);

    this.updatePossession(safeDt);
    ...
    updateBall(this.ball, safeDt);

    this.handleBallOwner(safeDt);

    this.handleBallPhysics(safeDt);

    this.handleTackles(safeDt);
    ...
    updateTeamMinutes(this.home, safeDt);
    updateTeamMinutes(this.away, safeDt);
    ...
    if (this.substitutionTimer > 7) {
      this.substitutionTimer = 0;

      aiSubstitute(this, this.home);
      aiSubstitute(this, this.away);
    }
  ...
  // ✅ nshya: yongera timer ya buri player
  tickPlayerActionTimers(dt) {
    for (const team of [
      this.home,
      this.away,
    ]) {
      for (const player of team.players) {
        ...
        player.lastActionAt += dt;
      }
    }
  }

  updatePossession() {
    const owner = this.getPlayer(
      this.ball.ownerId
    );

    if (owner) {
      this.lastPossessionTeam = owner.side;
    }
  }

  updatePossessionStats(dt) {
    if (this.lastPossessionTeam === "home") {
      this.home.stats.possessionSeconds += dt;
    } else {
      this.away.stats.possessionSeconds += dt;
    }
  }
  ...
  handleBallOwner() {
    ...
    moveBallWithOwner(this.ball, owner);

    this.lastTouchTeam = owner.side;
    this.lastTouchPlayerId = owner.id;

    const opponent = this.getOpponent(
      owner.side
    );

    const nearest = opponent.players
      .filter((p) => !p.redCard)
      .sort(
        (a, b) =>
          distance(a, owner) -
    ...
    this.checkAttackingDecision(owner);
  }

  checkAttackingDecision(player) {
    if (!player.hasBall) {
      return;
    }

    const team = this.getTeam(player.side);
    const opponent = this.getOpponent(
      player.side
    );
    ...
    // ✅ Gukoresha attemptShot import hejuru
    if (
      goalDistance < 360 &&
      lastAction > 1.4
    ) {
      const chance =
        0.18 + player.shooting / 400;
    ...
    // ✅ Gukoresha attemptPass import hejuru
    if (
      pressure < 90 &&
      lastAction > 1.0
    ) {
  ...
  checkPassReception() {
    ...
    if (closest && closestDistance < 20) {
      const previousTeam =
        this.lastTouchTeam;

      if (closest.side === previousTeam) {
        setBallOwner(this.ball, closest);

        this.lastPass = {
          receiverId: closest.id,
          team: closest.side,
        };
      ...
      const defendingTeam = this.getTeam(
        closest.side
      );
      ...
      if (receiver && Math.random() < 0.18) {
        setBallOwner(this.ball, closest);

        defendingTeam.stats.interceptions += 1;
        closest.interceptions += 1;
  ...
  checkShot() {
    const shooterTeam = this.lastShot
      ? this.getTeam(this.lastShot.team)
      : null;
  ...
  checkBoundaries() {
    ...
    const outsideLeft = this.ball.x < 0;
    const outsideRight =
      this.ball.x > FIELD.width;

    const outsideTop = this.ball.y < 0;
    const outsideBottom =
      this.ball.y > FIELD.height;

    if (outsideTop || outsideBottom) {
      this.ball.state = BALL_STATE.OUT;
  ...
  handleGoalLineRestart() {
    ...
    if (corner) {
      attackingTeam.stats.corners += 1;

      this.addEvent({
        type: EVENTS.CORNER,
        team: attackingTeam.side,
        text: `${attackingTeam.name} wins a corner`,
      });

      const cornerPlayer =
        attackingTeam.players.find((p) =>
          [
            "LW",
            "RW",
            "LM",
  ...
  scoreGoal(team, scorer) {
    ...
    if (
      this.lastPass &&
      this.lastPass.team === team.side &&
      this.lastPass.receiverId &&
      this.lastPass.receiverId !==
        scorer?.id
    ) {
      assistPlayer = this.getPlayer(
        this.lastPass.receiverId
      );
    ...
    this.addEvent({
      ...
      text: `${team.name} scores! ${
        scorer?.name ?? "Goal"
      }`,
    });

    this.ball.state = BALL_STATE.GOAL;
  ...
  checkNearbyDefender(
    ...
    const defender = defendingTeam.players
      .filter(
        (p) =>
          !p.redCard &&
          p.position !== "GK"
  ...
  addEvent({
    ...
    const event = {
      id: `${this.matchId}-${++this.eventCounter}`,
      ...
      relatedPlayerId:
        relatedPlayer?.id ?? null,
      relatedPlayerName:
        relatedPlayer?.name ?? null,
  ...
  getSnapshot() {
    return {
      ...
      home: {
        ...
        name: this.home.name,
        logo: this.home.logo,
        formation: this.home.formation,
        tactics: { ...this.home.tactics },
        score: this.score.home,
        stats: { ...this.home.stats },
        substitutionsUsed:
          this.home.substitutionsUsed,
        players: this.home.players.map(
          (p) => ({ ...p })
        ),
      },
      ...
      away: {
        ...
        name: this.away.name,
        logo: this.away.logo,
        formation: this.away.formation,
        tactics: { ...this.away.tactics },
        score: this.score.away,
        stats: { ...this.away.stats },
        substitutionsUsed:
          this.away.substitutionsUsed,
        players: this.away.players.map(
          (p) => ({ ...p })
        ),
      },
  ...
  serializeResult() {
    return {
      ...
      homeStats: { ...this.home.stats },
      awayStats: { ...this.away.stats },
